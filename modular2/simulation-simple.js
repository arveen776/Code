// Configuration
console.log('simulation-simple.js script loaded');
...
function initScene() {
    ...
    try {
        ...
        if (!container) {
            throw new Error('canvas-container element not found in DOM');
        }
...
function createCube(position, index) {
    if (!scene || !world) {
        console.error('Cannot create cube: scene or world not initialized');
        return null;
    }
    ...
    const body = new CANNON.Body({ mass: CUBE_MASS });
    body.addShape(shape);
    ...
    console.log(`Cube ${index} created at (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)})`);
    return cube;
}
...
function initializeCubes(count) {
    ...
    if (!scene || !world) {
        console.error('Cannot create cubes: scene or world not initialized');
        return;
    }

    console.log('Initializing', count, 'cubes...');
...
function setTargetShape(shapeName) {
    ...
    cubes.forEach((cube, index) => {
        if (index < targetPositions.length) {
            cube.targetPosition = new CANNON.Vec3(
...
function applyAutonomousMovement() {
    ...
    cubes.forEach((cube) => {
        ...
        if (currentPos.y < targetPos.y + 0.5) {
            ...
            body.applyLocalForce(
                new CANNON.Vec3(0, upwardForce, 0),
                new CANNON.Vec3(0, 0, 0)
            );
        }
...
function animate() {
    ...
    if (!scene || !camera || !renderer) {
        return; // Don't render if not initialized
    }

    // Step physics (only if world exists and has bodies)
    if (world && cubes.length > 0) {
        const timeStep = 1 / 60;
        world.step(timeStep);
    ...
    if (isAssembling && cubes.length > 0) {
        ...
        // Static camera view when not assembling
        camera.position.set(8, 8, 8);
        camera.lookAt(0, 0, 0);
    }
...
function resetSimulation() {
    // Remove all cubes (only if scene/world exist)
    if (scene && world) {
        cubes.forEach((cube) => {
...
function startAssembly() {
    console.log('Start assembly clicked');
    console.log('Scene:', scene);
    console.log('World:', world);
    
    // Check if scene is initialized
    if (!scene || !world) {
        console.error('Scene not initialized yet. Scene:', scene, 'World:', world);
        const statusEl = document.getElementById('status');
        if (statusEl) {
            const statusText = statusEl.textContent;
            alert('Please wait for the simulation to initialize...\n\nCurrent status: ' + statusText + '\n\nCheck the browser console (F12) for details.');
        } else {
    ...
    console.log('Creating', cubeCount, 'cubes for shape:', shapeName);
    ...
    console.log('Assembly started with', cubes.length, 'cubes');
}

// Wait for libraries to load, then initialize
let initAttempts = 0;
const MAX_INIT_ATTEMPTS = 100; // 10 seconds max (100 * 100ms)

function init() {
    initAttempts++;
    
    if (typeof THREE === 'undefined') {
        if (initAttempts % 10 === 0) { // Log every 10 attempts to reduce spam
            console.log(`Attempt ${initAttempts}: THREE.js not loaded yet...`);
        }
    ...
    if (typeof CANNON === 'undefined') {
        if (initAttempts % 10 === 0) { // Log every 10 attempts to reduce spam
            console.log(`Attempt ${initAttempts}: CANNON.js not loaded yet...`);
        }
        if (initAttempts < MAX_INIT_ATTEMPTS) {
            setTimeout(init, 100);
        } else {
            document.getElementById('status').textContent = 'Error: CANNON.js failed to load';
            document.getElementById('status').style.color = '#ff0000';
            console.error('CANNON.js failed to load after', MAX_INIT_ATTEMPTS, 'attempts');
            alert('CANNON.js physics library failed to load.\n\nPossible solutions:\n1. Check your internet connection\n2. Disable ad blockers\n3. Check firewall settings\n4. Try a different browser\n\nRefresh the page to retry.');
        }
        return;
    }

    try {
        console.log('Libraries loaded! THREE:', typeof THREE, 'CANNON:', typeof CANNON);
        console.log('Initializing scene...');
        document.getElementById('status').textContent = 'Initializing...';
        
        initScene();
        
        console.log('Scene initialized. Scene:', scene, 'World:', world);
        console.log('Starting animation loop...');
        animate();
        
        document.getElementById('status').textContent = 'Ready';
        document.getElementById('status').style.color = '#4CAF50';
        console.log('Simulation initialized successfully - Scene and World are ready');
        console.log('You can now click "Start Assembly"');
    } catch (error) {
...
function setupEventListeners() {
    ...
    if (startBtn) {
        ...
        newStartBtn.addEventListener('click', startAssembly);
